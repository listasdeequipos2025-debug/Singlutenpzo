...
export async function drawReceiptToCanvas(sale: Sale): Promise<HTMLCanvasElement> {
  ...
  if (sale.items && sale.items.length > 0) {
    for (const item of sale.items) {
      ...
      let prodName = item.productName || "Producto";
      if (prodName.length > 36) {
        prodName = prodName.substring(0, 34) + "...";
      }
      ctx.fillText(prodName, 42, y);

      ctx.textAlign = "center";
      ctx.fillText(String(item.quantity || 1), width - 140, y);

      const itemTotal = (item.salePrice || 0) * (item.quantity || 1);
      totalSalePriceSum += itemTotal;
      ctx.textAlign = "right";
      ctx.fillStyle = "#0284c7";
      ctx.font = "bold 12px system-ui, -apple-system, sans-serif";
      ctx.fillText(`$${itemTotal.toLocaleString()}`, width - 42, y);
      y += 22;
    }
  } else {
    ctx.textAlign = "left";
    ...
    const itemTotal = (sale.salePrice || 0) * (sale.quantity || 1);
    totalSalePriceSum = itemTotal;
    ctx.textAlign = "right";
    ctx.fillStyle = "#0284c7";
  ...
  const totalItem = totalSalePriceSum;
...
/**
 * Generates and returns the PNG base64 Data URL for the receipt.
 */
export async function getReceiptDataUrl(
  sale: Sale,
  domElement?: HTMLElement | null
): Promise<string> {
 ...
 * Returns the dataUrl so caller can also open a visual modal or preview if needed.
 */
export async function downloadReceiptImage(
  ...
  // 1. Try Web Share API with File (Most reliable on Redmi 14 / Android)
  try {
    ...
    if (navigator.canShare && navigator.canShare({ files: [file] })) {
      await navigator.share({
        ...
        text: `Estimado(a) ${sale.customerName}, le adjuntamos su Recibo de Venta.`,
      });
      return dataUrl;
    }
  } catch (e) {
    console.log("Web Share in downloadReceiptImage skipped/cancelled:", e);
  }

  // 2. Direct Anchor Click using Data URL
  try {
    ...
    console.warn("Direct download link trigger failed:", err);
  }
 ...
 * Returns { success, method, dataUrl }.
 */
export async function shareReceiptImage(
  ...
  try {
    ...
    // 1. Web Share API (Mobile native share sheet on Android / Redmi 14)
    if (navigator.canShare && navigator.canShare({ files: [file] })) {
      try {
        await navigator.share({
          files: [file],
        ...
        return { success: true, method: "web-share", dataUrl };
      } catch (e) {
    ...
    // Open WhatsApp chat text if callback provided
    if (onWhatsAppFallback) {
      onWhatsAppFallback();
    }

    return { success: true, method: imageCopied ? "clipboard+download" : "download", dataUrl };
  } catch (err) {
    console.error("shareReceiptImage error:", err);
    if (onWhatsAppFallback) onWhatsAppFallback();
    return { success: false, method: "error", dataUrl };
  }
}
