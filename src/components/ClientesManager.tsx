...
export default function ClientesManager({
  ...
  // Broadcast WhatsApp Modal States
  const [showBroadcastModal, setShowBroadcastModal] = useState(false);
  const [broadcastTarget, setBroadcastTarget] = useState<"todos" | "seleccionados" | "cliente">("todos");
  const [broadcastSingleCustomerCedula, setBroadcastSingleCustomerCedula] = useState<string>("");
  const [broadcastSearchTerm, setBroadcastSearchTerm] = useState<string>("");
  const [broadcastCopied, setBroadcastCopied] = useState(false);
  const DEFAULT_BROADCAST_MESSAGE = 
    `¡Hola, {nombre}! 😊 En SinGlutenpzo / Oriven esperamos que estés teniendo un excelente día.\n\n` +
    `Pasamos a saludarte y recordarte que seguimos a tu disposición con la mejor variedad de productos sin gluten y postres saludables para ti y tu familia. 🌾🚫\n\n` +
    `🛒 ¿Necesitas reponer tus productos favoritos esta semana?\n` +
    `Puedes consultarnos y realizar tu pedido directamente respondiendo a este mensaje.\n\n` +
    `🚚 Hacemos entregas rápidas y contamos con facilidades de pago.\n\n` +
    `¡Estamos a tu orden! ❤️`;
  const [broadcastMessage, setBroadcastMessage] = useState<string>(DEFAULT_BROADCAST_MESSAGE);
  ...
  // Broadcast WhatsApp Helper Functions
  const getPersonalizedBroadcastMessage = (customerName?: string) => {
    ...
    return broadcastMessage.replace(/\{nombre\}/gi, namePart);
  };

  const sendBroadcastWhatsAppToCustomer = (customer: { name: string; phone: string }) => {
    const cleanPhone = customer.phone.replace(/[^0-9]/g, "");
    if (!cleanPhone || cleanPhone === "N/A" || cleanPhone.length < 5) {
      alert(`El cliente ${customer.name} no posee un número de teléfono válido registrado para WhatsApp.`);
      return;
    }
    ...
    const text = getPersonalizedBroadcastMessage(customer.name);
    const encodedText = encodeURIComponent(text);
    const url = `https://api.whatsapp.com/send?phone=${finalPhone}&text=${encodedText}`;
    window.open(url, "_blank");
  };

  const handleOpenBroadcastForAll = () => {
    setBroadcastTarget("todos");
    setShowBroadcastModal(true);
  };

  const handleOpenBroadcastForSelected = () => {
    if (selectedCustomerCedulas.length === 0) {
      alert("Por favor selecciona al menos un cliente en el directorio.");
      return;
    }
    setBroadcastTarget("seleccionados");
    setShowBroadcastModal(true);
  };

  const handleOpenBroadcastForCustomer = (customer: CustomerGroup) => {
    setBroadcastTarget("cliente");
    setBroadcastSingleCustomerCedula(customer.cedula);
    setShowBroadcastModal(true);
  };

  const handleCopyBroadcastGeneralText = async () => {
    const generalText = broadcastMessage
      .replace(/,\s*\{nombre\}/gi, "")
      .replace(/\{nombre\}/gi, "");
    try {
  ...
  const handleDownloadReceiptImage = async () => {
    ...
    try {
      const dataUrl = await downloadReceiptImage(selectedReceiptSale, receiptRef.current);
      if (dataUrl) {
  ...
  const handleShareReceiptImage = async () => {
    ...
    setIsGeneratingImage(true);
    try {
      const res = await shareReceiptImage(selectedReceiptSale, receiptRef.current, () => {
    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="flex flex-col xl:flex-row xl:items-center xl:justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2 self-start xl:self-auto">
          {/* Action buttons */}
          <button
            onClick={handleOpenBroadcastForAll}
            className="flex items-center space-x-1.5 px-4 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs uppercase tracking-wider rounded-xl transition-all shadow-md hover:shadow-emerald-500/25"
            title="Enviar ofertas y mensajes masivos por WhatsApp a clientes"
          >
          ...
          {selectedCustomerCedulas.length > 0 && (
            <div className="bg-blue-50 border border-blue-100 rounded-3xl p-4 flex flex-col sm:flex-row items-center justify-between gap-4 animate-fade-in shadow-sm">
              ...
              <div className="flex flex-wrap space-x-2 w-full sm:w-auto">
                <button
                  type="button"
                  onClick={handleOpenBroadcastForSelected}
                  className="flex-1 sm:flex-initial flex items-center justify-center space-x-1.5 px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs uppercase tracking-wider rounded-xl transition-all shadow-sm"
                  title="Enviar oferta o mensaje promocional por WhatsApp a los clientes seleccionados"
                >
                  <MessageSquare className="h-4 w-4" />
                  <span>📢 Oferta WhatsApp ({selectedCustomerCedulas.length})</span>
                </button>
                <button
                  type="button"
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-50 text-slate-400 text-[10px] uppercase font-black tracking-wider border-b border-slate-100">
                    ...
                    <th className="p-4 text-center">Oferta WhatsApp</th>
                  </tr>
                </thead>
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="p-4 text-center" onClick={(e) => e.stopPropagation()}>
                            <button
                              type="button"
                              onClick={() => handleOpenBroadcastForCustomer(c)}
                              title="Enviar oferta o mensaje de productos a este cliente por WhatsApp"
                              className="inline-flex items-center space-x-1 px-3 py-1.5 bg-emerald-50 hover:bg-emerald-600 text-emerald-700 hover:text-white rounded-xl text-xs font-bold transition-all border border-emerald-200/60 shadow-sm"
                            >
            ...
            <div className="flex-1 p-6 overflow-y-auto space-y-6">
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-3xl p-4 flex items-center justify-between gap-4 animate-fade-in">
                ...
                <div className="flex flex-wrap gap-2">
                  <button
                    type="button"
                    onClick={() => handleOpenBroadcastForCustomer(selectedCustomer)}
                    className="flex items-center space-x-1 px-3 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-xs font-bold transition-all shadow-sm"
                    title="Enviar oferta o mensaje de productos a este cliente por WhatsApp"
                  >
                    <MessageSquare className="h-3.5 w-3.5" />
                    <span>Enviar Oferta</span>
                ...
                {selectedReceiptSale.items && selectedReceiptSale.items.length > 0 ? (
                  selectedReceiptSale.items.map((item, idx) => (
                    <div key={idx} className="flex justify-between text-xs font-bold text-slate-800">
                      <span className="break-words max-w-[200px]">{item.productName}</span>
                      <div className="space-x-12 flex">
                        <span>{item.quantity}</span>
                        <span className="font-extrabold text-blue-950">${(item.salePrice * item.quantity).toLocaleString()}</span>
                      </div>
                    </div>
                  ))
                ) : (
      ...
      {/* Generated Receipt Image Modal (Optimized for Android / Redmi 14) */}
      {previewDataUrl && selectedReceiptSale && (
        <ReceiptImageModal
          sale={selectedReceiptSale}
          dataUrl={previewDataUrl}
          onClose={() => setPreviewDataUrl(null)}
          onWhatsAppText={() => handleShareWhatsApp(selectedReceiptSale)}
        />
      )}

      {/* Broadcast WhatsApp / Offer Products Modal */}
      {showBroadcastModal && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-[95] flex items-center justify-center p-3 sm:p-5 overflow-y-auto">
          <div className="bg-white rounded-3xl w-full max-w-2xl p-5 sm:p-7 shadow-2xl relative border border-slate-100 space-y-6 my-auto max-h-[92vh] overflow-y-auto animate-fade-in">
            {/* Modal Header */}
            <div className="flex items-start justify-between border-b border-slate-100 pb-4">
              <div className="flex items-center space-x-3">
                <div className="h-12 w-12 rounded-2xl bg-emerald-100 text-emerald-700 flex items-center justify-center font-black shadow-sm">
                  <MessageSquare className="h-6 w-6" />
                </div>
                <div>
                  <h3 className="text-xl font-black text-slate-900 tracking-tight flex items-center space-x-2">
                    <span>📢 Difusión & Oferta por WhatsApp</span>
                  </h3>
                  <p className="text-xs text-slate-500 font-semibold">
                    Envía promociones y recordatorios a tus clientes registrados en SinGlutenpzo / Oriven
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-3 gap-2 bg-slate-100 p-1.5 rounded-2xl border border-slate-200/60">
                ...
                <button
                  ...
                  onClick={() => {
                    if (selectedCustomerCedulas.length === 0) {
                      alert("No hay clientes seleccionados. Selecciona clientes en el directorio primero o usa 'Todos'.");
                      return;
                    }
                    setBroadcastTarget("seleccionados");
                  }}
                  className={`py-2 px-3 rounded-xl text-xs font-bold transition-all flex items-center justify-center space-x-1.5 ${
                    broadcastTarget === "seleccionados"
                      ? "bg-emerald-600 text-white shadow-sm font-extrabold"
                      : "text-slate-600 hover:text-slate-900"
                  }`}
                >
                  <CheckCircle className="h-3.5 w-3.5" />
                  <span>Seleccionados ({selectedCustomerCedulas.length})</span>
                </button>

                <button
                  type="button"
                  onClick={() => setBroadcastTarget("cliente")}
                  className={`py-2 px-3 rounded-xl text-xs font-bold transition-all flex items-center justify-center space-x-1.5 ${
                    broadcastTarget === "cliente"
                      ? "bg-emerald-600 text-white shadow-sm font-extrabold"
                      : "text-slate-600 hover:text-slate-900"
                  }`}
                >
              ...
              {/* If Single Customer option, show select dropdown */}
              {broadcastTarget === "cliente" && (
                <div className="pt-2">
                  <select
                    value={broadcastSingleCustomerCedula}
                    onChange={(e) => setBroadcastSingleCustomerCedula(e.target.value)}
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <label className="text-xs font-black uppercase text-slate-400 tracking-wider block">
                  2. Mensaje promocional a enviar:
                </label>
                <button
                  type="button"
                  onClick={() => setBroadcastMessage(DEFAULT_BROADCAST_MESSAGE)}
                  className="text-[10px] font-bold text-emerald-700 hover:underline flex items-center space-x-1"
                >
                  <span>Restablecer Mensaje Original</span>
                </button>
              </div>
              ...
              <textarea
                ...
                className="w-full p-4 bg-slate-50 border border-slate-200 focus:border-emerald-500 focus:bg-white focus:ring-2 focus:ring-emerald-100 rounded-2xl text-xs font-medium text-slate-800 leading-relaxed outline-none transition-all shadow-inner font-mono"
              />

              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 text-[11px] text-slate-500 bg-emerald-50/60 border border-emerald-100 p-2.5 rounded-xl">
                <span>
                  💡 Usa <code className="bg-emerald-100 font-bold text-emerald-900 px-1 py-0.5 rounded">{`{nombre}`}</code> en el texto para insertar automáticamente el nombre de cada cliente.
                </span>
                <button
                  type="button"
                  onClick={handleCopyBroadcastGeneralText}
                  className="px-3 py-1 bg-white hover:bg-emerald-100 text-emerald-800 border border-emerald-200 rounded-lg font-bold text-[10px] uppercase tracking-wider transition-all shadow-sm flex items-center space-x-1 whitespace-nowrap self-end sm:self-auto"
                >
                  <FileText className="h-3 w-3" />
                  <span>{broadcastCopied ? "¡Copiado! ✓" : "Copiar p/ Lista Difusión"}</span>
                </button>
              </div>
            </div>

            {/* Target Clients List & Action Buttons */}
            <div className="space-y-3 pt-2">
              <div className="flex items-center justify-between">
                <label className="text-xs font-black uppercase text-slate-400 tracking-wider block">
                  3. Enviar por WhatsApp individualmente:
                </label>
                <div className="relative w-48">
                  ...
                  <input
                    ...
                    className="w-full pl-8 pr-3 py-1 bg-slate-50 border border-slate-200 rounded-xl text-[11px] focus:outline-none focus:ring-1 focus:ring-emerald-500"
                  />
                </div>
              </div>

              {/* Compute filtered list for modal */}
              {(() => {
                let list: CustomerGroup[] = [];
                if (broadcastTarget === "todos") {
                  list = customers;
                } else if (broadcastTarget === "seleccionados") {
                  list = customers.filter((c) => selectedCustomerCedulas.includes(c.cedula));
                } else if (broadcastTarget === "cliente") {
                  list = customers.filter((c) => c.cedula === broadcastSingleCustomerCedula);
                }

                if (broadcastSearchTerm) {
                ...
                if (list.length === 0) {
                  return (
                    <div className="bg-slate-50 border border-slate-100 rounded-2xl p-6 text-center text-xs font-bold text-slate-400">
                      No hay clientes en este grupo o no coinciden con la búsqueda.
                    </div>
                  );
                }
                ...
                return (
                  <div className="space-y-2 max-h-60 overflow-y-auto pr-1">
                    {list.map((c) => {
                      ...
                      const isValidPhone = cleanPhone && cleanPhone !== "N/A" && cleanPhone.length >= 5;

                      return (
                        <div
                          key={c.cedula}
                          className="bg-slate-50 border border-slate-100 hover:border-emerald-200 rounded-2xl p-3 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 transition-colors"
                        >
                          <div className="flex items-center space-x-3">
                            <div className="h-9 w-9 rounded-full bg-emerald-100 text-emerald-800 flex items-center justify-center font-black text-sm">
                              {c.name.charAt(0).toUpperCase()}
                            </div>
                            <div>
                              <p className="text-xs font-black text-slate-800">{c.name}</p>
                              <p className="text-[10px] text-slate-500 font-semibold">
                                Tel: <span className="font-mono text-slate-700">{c.phone}</span> | Cédula: {c.cedula}
                              </p>
                            </div>
                          </div>
                          ...
                          <div className="flex items-center space-x-2 w-full sm:w-auto justify-end">
                            {isValidPhone ? (
                              <button
                                type="button"
                                onClick={() => sendBroadcastWhatsAppToCustomer(c)}
                                className="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-xs font-bold transition-all shadow-sm flex items-center space-x-1.5"
                              >
                                <MessageSquare className="h-3.5 w-3.5" />
                                <span>Enviar WhatsApp</span>
                              </button>
                            ) : (
                              <span className="text-[10px] text-amber-600 font-bold bg-amber-50 px-2.5 py-1 rounded-xl border border-amber-200">
                                Sin teléfono válido
                              </span>
                            )}
                          </div>
            ...
            <div className="pt-4 border-t border-slate-100 flex items-center justify-between">
              <button
                type="button"
                onClick={() => setShowBroadcastModal(false)}
                className="px-5 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-2xl text-xs font-bold uppercase tracking-wider transition-colors"
              >
                Cerrar
              </button>
              <button
                type="button"
                onClick={handleCopyBroadcastGeneralText}
                className="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-2xl text-xs font-extrabold uppercase tracking-wider transition-all shadow-md shadow-emerald-200 flex items-center space-x-2"
              >
                <FileText className="h-4 w-4" />
                <span>{broadcastCopied ? "¡Texto Copiado!" : "Copiar Texto de Oferta"}</span>
              </button>
            </div>
          </div>
        </div>
