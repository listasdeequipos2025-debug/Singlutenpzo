        ...
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-[80] flex items-center justify-center p-2 sm:p-4 overflow-y-auto">
          <div className="bg-white rounded-2xl sm:rounded-3xl w-full max-w-lg p-4 sm:p-6 shadow-2xl relative border border-slate-100 my-auto max-h-[92vh] overflow-y-auto">
            ...
            <form onSubmit={handleRegisterSaleSubmit} className="space-y-5">
                ...
                <div className="space-y-3.5 bg-slate-50 p-4 rounded-2xl border border-slate-200/60 animate-fade-in">
                  ...
                  <div className="pt-2">
                    <button
                      ...
                      className="w-full py-2.5 px-4 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs uppercase tracking-wider rounded-xl transition-all shadow-sm flex items-center justify-center space-x-2 cursor-pointer"
                    >
                      <Plus className="h-4 w-4" />
              ...
              {/* Shopping Cart List of Added Products */}
              {saleItems.length > 0 && (
                <div className="bg-slate-50 border border-slate-200/80 p-4 rounded-2xl space-y-3 animate-fade-in">
                  <div className="flex justify-between items-center pb-2 border-b border-slate-200">
                    <span className="text-xs font-black text-slate-800 uppercase tracking-wider flex items-center space-x-1.5">
                      ...
                      <span>Productos en esta Venta ({saleItems.length})</span>
                    </span>
                    <span className="text-xs font-extrabold text-blue-950 bg-blue-100 px-2.5 py-1 rounded-lg">
                      Subtotal: ${saleItems.reduce((acc, i) => acc + (i.salePrice * i.quantity), 0).toLocaleString()}
                    </span>
                  </div>
                      ...
                      <div key={idx} className="flex items-center justify-between bg-white p-3 rounded-xl border border-slate-200 text-xs shadow-2xs">
                        <div className="space-y-0.5 max-w-[180px] sm:max-w-[240px]">
                          ...
                          <p className="text-[10px] text-slate-500">
                            Unitario: ${item.salePrice.toLocaleString()} | Subtotal: <span className="font-bold text-blue-950">${(item.salePrice * item.quantity).toLocaleString()}</span>
                          </p>
                        </div>
