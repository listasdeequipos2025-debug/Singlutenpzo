...
import { Product, Sale, SaleItem, ProductCategory } from "../types";
import {
...
export default function SalesManager({
  ...
  const cartTotal = saleItems.reduce((acc, item) => acc + (item.salePrice * item.quantity), 0);
  const pendingUnit = parseFloat(salePriceInput) || 0;
  const pendingQty = parseInt(quantity || "0", 10);
  const pendingTotal = (selectedProductId && chosenProduct) ? pendingUnit * pendingQty : 0;
  const totalSaleAmount = saleItems.length > 0 ? (cartTotal + pendingTotal) : (pendingTotal || cartTotal);

  const handleAddItemToSale = () => {
    setFormError("");
    if (!selectedProductId || !chosenProduct) {
      setFormError("Por favor, selecciona un producto para agregar.");
      return;
    }
    ...
    const finalPrice = parseFloat(salePriceInput);
    if (isNaN(finalPrice) || finalPrice <= 0) {
      setFormError("Ingresa un precio de venta válido mayor a cero.");
      return;
    }
    ...
    if (!isNaN(refVal)) {
      ...
      profit = (finalPrice - chosenProduct.costPrice) * qty;
    } else {
      profit = (chosenProduct.referenceProfit || 0) * qty;
    }

    setSaleItems((prev) => {
      const existingIdx = prev.findIndex((i) => i.productId === chosenProduct.id);
      if (existingIdx >= 0) {
        const copy = [...prev];
        const existing = copy[existingIdx];
        const newQty = existing.quantity + qty;
        const unitProfit = existing.quantity > 0 ? existing.profit / existing.quantity : profit / qty;
        copy[existingIdx] = {
          ...existing,
          quantity: newQty,
        ...
        const newItem: SaleItem = {
          ...
          profit: profit,
          referenceProfit: !isNaN(refVal) ? refVal : undefined
        };
        return [...prev, newItem];
      }
    });
  ...
  const handleUpdateItemQuantity = (index: number, newQty: number) => {
    if (newQty <= 0) {
      handleRemoveItemFromSale(index);
      return;
    }
    setSaleItems((prev) => {
      const copy = [...prev];
  ...
  const handleRegisterSaleSubmit = async (e: React.FormEvent) => {
    ...
    // If user filled in a product but didn't click "+ Añadir", auto-add it
    if (selectedProductId && chosenProduct) {
      const qty = parseInt(quantity, 10);
      const finalPrice = parseFloat(salePriceInput);
      if (!isNaN(qty) && qty > 0 && !isNaN(finalPrice) && finalPrice > 0) {
        let profit = 0;
        const refVal = parseFloat(referenceInput);
        if (!isNaN(refVal)) {
          profit = refVal * qty;
        ...
        const existingIdx = finalItems.findIndex(i => i.productId === chosenProduct.id);
        if (existingIdx >= 0) {
    ...
    if (finalItems.length === 0) {
      setFormError("Por favor, selecciona y añade al menos un producto a la venta.");
      return;
    }

    const total = finalItems.reduce((acc, i) => acc + (i.salePrice * i.quantity), 0);
    const totalCost = finalItems.reduce((acc, i) => acc + (i.costPrice * i.quantity), 0);
    const totalProfit = finalItems.reduce((acc, i) => acc + i.profit, 0);
    const totalQty = finalItems.reduce((acc, i) => acc + i.quantity, 0);
    ...
    const summaryProductName = finalItems.length === 1
      ? finalItems[0].productName
      : finalItems.map(i => `${i.productName} (x${i.quantity})`).join(", ");

    const saleData = {
      productId: finalItems[0].productId,
      productName: summaryProductName,
      category: finalItems[0].category,
      quantity: totalQty,
      salePrice: totalQty > 0 ? total / totalQty : 0,
      costPrice: totalQty > 0 ? totalCost / totalQty : 0,
      profit: totalProfit,
      items: finalItems,
        ...
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-[80] flex items-center justify-center p-2 sm:p-4 overflow-y-auto">
          <div className="bg-white rounded-2xl sm:rounded-3xl w-full max-w-lg p-4 sm:p-6 shadow-2xl relative border border-slate-100 my-auto max-h-[92vh] overflow-y-auto">
            ...
            <form onSubmit={handleRegisterSaleSubmit} className="space-y-5">
              {/* Shopping Cart List of Added Products - Prominent Top Position */}
              <div className="bg-slate-50 border border-slate-200 p-4 rounded-2xl space-y-3">
                <div className="flex justify-between items-center pb-2 border-b border-slate-200">
                  <span className="text-xs font-black text-slate-800 uppercase tracking-wider flex items-center space-x-1.5">
                    ...
                    <span>Productos en esta Factura / Recibo ({saleItems.length})</span>
                  </span>
                  {saleItems.length > 0 && (
                    <span className="text-xs font-extrabold text-emerald-950 bg-emerald-100 px-2.5 py-1 rounded-lg border border-emerald-200">
                      Total: ${saleItems.reduce((acc, i) => acc + (i.salePrice * i.quantity), 0).toLocaleString()}
                    </span>
                  )}
                </div>

                {saleItems.length === 0 ? (
                  <div className="text-center py-4 px-2 text-slate-500 text-xs space-y-1">
                    <p className="font-bold text-slate-700">🛒 Aún no has añadido productos al recibo.</p>
                    <p className="text-[11px] text-slate-400">Selecciona un producto abajo y haz clic en "Añadir a la Factura".</p>
                  </div>
                ) : (
                  <div className="space-y-2 max-h-52 overflow-y-auto pr-1">
                    {saleItems.map((item, idx) => (
                      <div key={idx} className="flex items-center justify-between bg-white p-3 rounded-xl border border-slate-200 text-xs shadow-2xs">
                        <div className="space-y-0.5 max-w-[180px] sm:max-w-[240px]">
                          ...
                          <p className="text-[10px] text-slate-500">
                            ${item.salePrice.toLocaleString()} c/u | Subtotal: <span className="font-bold text-blue-950">${(item.salePrice * item.quantity).toLocaleString()}</span>
                          </p>
                        </div>
                        ...
                        <div className="flex items-center space-x-2">
                          <div className="flex items-center space-x-1 bg-slate-100 p-1 rounded-lg border border-slate-200">
                            <button
                              ...
                              className="w-5 h-5 flex items-center justify-center bg-white hover:bg-slate-200 text-slate-700 font-bold rounded text-xs transition-colors cursor-pointer"
              ...
              {/* Add Product Block */}
              <div className="bg-emerald-50/50 border border-emerald-200/80 p-4 rounded-2xl space-y-3.5">
                <div className="space-y-1">
                  ...
                  <select
                    ...
                    className="w-full p-3 bg-white border border-slate-300 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 rounded-xl text-sm font-bold text-slate-800 transition-all outline-none cursor-pointer shadow-2xs"
                  >
                    <option value="">
                      {saleItems.length > 0 ? "+ Seleccionar otro producto para añadir..." : "-- Selecciona un producto del catálogo --"}
                    </option>
                    {products.map((p) => (
                ...
                {chosenProduct && (
                  <div className="space-y-3 pt-1 border-t border-emerald-200/60 animate-fade-in">
                    <div className="bg-white p-3 rounded-xl border border-emerald-100 flex items-center justify-between text-xs font-bold text-slate-700">
                      <div>
                        <p className="text-emerald-900 font-extrabold">{chosenProduct.name}</p>
                        <p className="text-[10px] text-slate-500 capitalize">Categoría: {chosenProduct.category}</p>
                      </div>
                      <div className="text-right">
                        <p className="text-[10px] text-slate-400">Stock Actual</p>
                        <p className="text-sm font-black text-emerald-700">{chosenProduct.stock} uds.</p>
                      </div>
                    </div>

                    {/* Quantity & Price structure row */}
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div className="space-y-1">
                        <label className="text-[11px] font-bold text-slate-600 uppercase">Cantidad a Llevar</label>
                        <input
                          type="number"
                          min="1"
                          required={saleItems.length === 0}
                          ...
                          className="w-full p-2.5 bg-white border border-slate-300 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 rounded-xl text-sm font-bold text-slate-900 outline-none"
                        />
                      </div>

                      <div className="space-y-1">
                        <label className="text-[11px] font-bold text-slate-600 uppercase">Estructura de Precio</label>
                        <div className="grid grid-cols-3 gap-1">
                          <button
                            type="button"
                            onClick={() => setPriceType("detal")}
                            className={`py-2 px-1 rounded-lg text-[9px] font-bold uppercase transition-all border ${
                              priceType === "detal"
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div className="space-y-1">
                        <label className="text-[11px] font-bold text-slate-600 uppercase">Precio Unitario Facturado ($)</label>
                        <input
                          type="number"
                          step="0.01"
                          required={saleItems.length === 0}
                          ...
                          className="w-full p-2.5 bg-white border border-slate-300 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 rounded-xl text-xs font-extrabold text-slate-900 outline-none"
                        />
                      </div>

                      <div className="space-y-1">
                        <label className="text-[11px] font-bold text-slate-500 uppercase">Ref. Ganancia (Interno, Opcional)</label>
                        <input
                          type="number"
                          step="0.01"
                    ...
                    <button
                      ...
                      className="w-full py-3 px-4 bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold text-xs uppercase tracking-wider rounded-xl transition-all shadow-md flex items-center justify-center space-x-2 cursor-pointer active:scale-[0.99]"
                    >
                      <Plus className="h-4 w-4" />
                      <span>+ AGREGAR "{chosenProduct.name.toUpperCase()}" AL RECIBO</span>
                    </button>
                  </div>
                )}
              </div>
              ...
              {totalSaleAmount > 0 && (
                <div className="border-t border-slate-200 pt-4 flex justify-between items-center text-sm font-bold">
                  <span className="text-slate-600">
                    Monto Total a Facturar {saleItems.length > 0 ? `(${saleItems.length} producto(s) en la lista)` : ""}:
                  </span>
            ...
            <div ref={receiptRef} className="border-2 border-slate-100 rounded-2xl p-5 space-y-4 bg-white">
              ...
              <div className="border-t border-slate-200/80 pt-3 flex flex-col space-y-1.5">
                ...
                <div className="border-t border-slate-100 pt-2 flex flex-col space-y-1">
                  {(() => {
                    const totalSale = selectedReceiptSale.items && selectedReceiptSale.items.length > 0
                      ? selectedReceiptSale.items.reduce((acc, i) => acc + (i.salePrice * i.quantity), 0)
                      : selectedReceiptSale.salePrice * selectedReceiptSale.quantity;
                    
                    const paidAmountVal = selectedReceiptSale.paidAmount !== undefined ? selectedReceiptSale.paidAmount : totalSale;
                    ...
                    return (
                      <>
                        <div className="flex justify-between w-full text-xs font-bold text-slate-500">
                          ...
                          <span className="text-slate-900 font-extrabold">${totalSale.toLocaleString()}</span>
                        </div>
                        <div className="flex justify-between w-full text-xs font-bold text-emerald-600">
                          <span>Monto Pagado (Totalizado):</span>
                          <span className="font-extrabold">${paidAmountVal.toLocaleString()}</span>
                        </div>
